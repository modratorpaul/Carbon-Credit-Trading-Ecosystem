...
(define-public (cancel-sell-order (order-id uint))
  (let
    ...
    (ok (map-set sell-orders
      { order-id: order-id }
      (merge order { status: "cancelled" })))))

(define-public (cancel-buy-order (order-id uint))
  (let
    ((order (unwrap! (map-get? buy-orders { order-id: order-id }) err-invalid-order)))
    (asserts! (is-eq tx-sender (get buyer order)) err-not-authorized)
    (asserts! (is-eq (get status order) "active") err-not-authorized)
    (ok (map-set buy-orders
      { order-id: order-id }
      (merge order { status: "cancelled" })))))

(define-public (execute-trade (sell-order-id uint) (buy-order-id uint))
  (let
    ((sell-order (unwrap! (map-get? sell-orders { order-id: sell-order-id }) err-invalid-order))
     (buy-order (unwrap! (map-get? buy-orders { order-id: buy-order-id }) err-invalid-order)))
    (asserts! (and (is-eq (get status sell-order) "active") (is-eq (get status buy-order) "active")) err-not-authorized)
    (asserts! (and (>= (get amount buy-order) (get amount sell-order)) (>= (get price buy-order) (get price sell-order))) err-not-authorized)
    (ok (begin
      (map-set sell-orders
        { order-id: sell-order-id }
        (merge sell-order { status: "completed" }))
      (map-set buy-orders
        { order-id: buy-order-id }
        (merge buy-order { status: "completed" }))))))

(define-read-only (get-sell-order (order-id uint))
  (ok (unwrap! (map-get? sell-orders { order-id: order-id }) err-invalid-order)))

(define-read-only (get-buy-order (order-id uint))
  (ok (unwrap! (map-get? buy-orders { order-id: order-id }) err-invalid-order)))
